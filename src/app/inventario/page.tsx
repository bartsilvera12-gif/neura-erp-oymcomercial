...
export default function InventarioPage() {
  ...
  // Resumen del listado visible. Solo los productos que controlan stock entran en
  // valorizado / bajo / disponibles; los que no lo controlan se cuentan únicamente
  // en "Total productos".
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard compact label="Stock bajo" value={String(resumen.bajo)}
          hint="≤ stock mínimo" />
        <StatCard compact label="Con stock disponible"
          ...
          <table className="w-full min-w-[780px] lg:min-w-0 text-left text-sm">
            ...
            <tbody>
              {productos.map((p) => {
                ...
                // "Sin control" es el producto vendible que no mueve stock. Los
                // insumos sí tienen stock real aunque controla_stock=false.
                const sinControl =
                    ...
                    <td className="py-4 pr-4 font-medium text-gray-800">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {(() => {
                          ...
                          // Mixto/Insumo se siguen mostrando; Vendible no lleva chip
                          // porque es el caso normal y etiquetaría a casi todo.
                          ...
                          <button
                            ...
                            onClick={async () => {
                              const ok = window.confirm(
                                `¿Eliminar "${p.nombre}" (${p.sku})?\n\nEl producto deja de aparecer en Inventario y en la caja. Las ventas, compras y movimientos anteriores se conservan intactos.`
                              );
                              if (!ok) return;
                              try {
                            ...
                            className="inline-flex items-center justify-center min-h-[40px] rounded-md border border-rose-200 bg-white px-2.5 py-1.5 text-xs font-medium text-rose-600 hover:border-rose-300 hover:bg-rose-50 transition-colors"
                            title="Eliminar producto (soft-delete)"
