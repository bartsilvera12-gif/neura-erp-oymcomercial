...
-- 1) Ítems del recibo
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS oymcomercial.recibos_dinero_items (
  id                    uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  recibo_id             uuid NOT NULL REFERENCES oymcomercial.recibos_dinero(id) ON DELETE CASCADE,
  empresa_id            uuid NOT NULL REFERENCES oymcomercial.empresas(id) ON DELETE CASCADE,
  cuenta_por_cobrar_id  uuid REFERENCES oymcomercial.cuentas_por_cobrar(id) ON DELETE RESTRICT,
  ...
  cobro_cliente_id      uuid REFERENCES oymcomercial.cobros_clientes(id) ON DELETE RESTRICT,
  factura_id            uuid REFERENCES oymcomercial.facturas(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_recibo_item_recibo ON oymcomercial.recibos_dinero_items (recibo_id);
CREATE INDEX IF NOT EXISTS idx_recibo_item_cxc    ON oymcomercial.recibos_dinero_items (cuenta_por_cobrar_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_recibo_item_cobro
  ON oymcomercial.recibos_dinero_items (cobro_cliente_id) WHERE cobro_cliente_id IS NOT NULL;

ALTER TABLE oymcomercial.recibos_dinero_items ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS recibo_items_empresa_isolation ON oymcomercial.recibos_dinero_items;
CREATE POLICY recibo_items_empresa_isolation ON oymcomercial.recibos_dinero_items
...
DROP INDEX IF EXISTS oymcomercial.uq_recibos_empresa_numero;
CREATE UNIQUE INDEX uq_recibos_empresa_sucursal_numero
  ON oymcomercial.recibos_dinero (empresa_id, sucursal_id, numero_recibo);
...
BEGIN
  SELECT count(*) INTO v_tab FROM information_schema.tables
   WHERE table_schema='oymcomercial' AND table_name='recibos_dinero_items';
  IF v_tab <> 1 THEN RAISE EXCEPTION 'ABORT: no se creó recibos_dinero_items'; END IF;

  SELECT count(*) INTO v_idx FROM pg_indexes
   WHERE schemaname='oymcomercial' AND indexname='uq_recibos_empresa_sucursal_numero';
