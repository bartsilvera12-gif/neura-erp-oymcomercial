-- Módulo Presupuestos (solo schema oymcomercial). Idempotente.
...
-- 1) presupuestos ----------------------------------------------------------
CREATE TABLE IF NOT EXISTS oymcomercial.presupuestos (
...
ALTER TABLE oymcomercial.presupuestos
...
CREATE TABLE IF NOT EXISTS oymcomercial.presupuesto_items (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  empresa_id      uuid NOT NULL,
  presupuesto_id  uuid NOT NULL REFERENCES oymcomercial.presupuestos(id) ON DELETE CASCADE,
  producto_id     uuid,
...
CREATE INDEX IF NOT EXISTS idx_presupuestos_empresa_fecha ON oymcomercial.presupuestos (empresa_id, fecha DESC);
CREATE INDEX IF NOT EXISTS idx_presupuestos_estado ON oymcomercial.presupuestos (empresa_id, estado);
CREATE INDEX IF NOT EXISTS idx_presupuesto_items_presupuesto ON oymcomercial.presupuesto_items (presupuesto_id);
...
WHERE NOT EXISTS (SELECT 1 FROM oymcomercial.modulos WHERE slug = 'presupuestos');
