...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    EXECUTE format('DROP TRIGGER IF EXISTS trg_heredar_sucursal ON oymcomercial.%I', t);
    EXECUTE format(
      'CREATE TRIGGER trg_heredar_sucursal BEFORE INSERT ON oymcomercial.%I
         FOR EACH ROW EXECUTE FUNCTION oymcomercial.fn_heredar_sucursal_id()', t);
  END LOOP;
END $$;
...
-- ---------------------------------------------------------------------------
UPDATE oymcomercial.factura_electronica fe
...
UPDATE oymcomercial.producciones pr
   SET sucursal_id = p.sucursal_id
  FROM oymcomercial.productos p
 WHERE pr.producto_id = p.id AND pr.sucursal_id IS NULL AND p.sucursal_id IS NOT NULL;

UPDATE oymcomercial.cobros_clientes co
   SET sucursal_id = c.sucursal_id
  FROM oymcomercial.cuentas_por_cobrar c
...
BEGIN
  SELECT s.id INTO v_suc FROM oymcomercial.sucursales s WHERE s.es_principal LIMIT 1;
  FOREACH t IN ARRAY ARRAY[
    'factura_electronica','sifen_jobs','movimientos_inventario',
    'producciones','cobros_clientes','recibos_dinero'
  ] LOOP
    EXECUTE format('UPDATE oymcomercial.%I SET sucursal_id = $1 WHERE sucursal_id IS NULL', t)
      USING v_suc;
  END LOOP;
END $$;
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    EXECUTE format('SELECT count(*) FROM oymcomercial.%I WHERE sucursal_id IS NULL', t) INTO n;
    total := total + n;
