...
--
-- Backup previo: /root/backup_origen_2026-07-19_2028.sql (5.2 MB,
...
DECLARE n int;
BEGIN
  SELECT count(*) INTO n FROM oymcomercial.empresas;
  IF n <> 1 THEN
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS oymcomercial.sucursales (
...
CREATE UNIQUE INDEX IF NOT EXISTS sucursales_una_principal_uq
  ON oymcomercial.sucursales (empresa_id) WHERE es_principal;

COMMENT ON TABLE oymcomercial.sucursales IS
...
INSERT INTO oymcomercial.sucursales
...
CREATE TABLE IF NOT EXISTS oymcomercial.usuario_sucursales (
  usuario_id   uuid NOT NULL REFERENCES oymcomercial.usuarios(id) ON DELETE CASCADE,
  sucursal_id  uuid NOT NULL REFERENCES oymcomercial.sucursales(id) ON DELETE CASCADE,
  empresa_id   uuid NOT NULL REFERENCES oymcomercial.empresas(id) ON DELETE CASCADE,
  created_at   timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (usuario_id, sucursal_id)
);
CREATE INDEX IF NOT EXISTS idx_usuario_sucursales_sucursal
  ON oymcomercial.usuario_sucursales (sucursal_id);

ALTER TABLE oymcomercial.usuarios
  ADD COLUMN IF NOT EXISTS sucursal_predeterminada_id uuid
    REFERENCES oymcomercial.sucursales(id) ON DELETE SET NULL;
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    EXECUTE format(
      'ALTER TABLE oymcomercial.%I ADD COLUMN IF NOT EXISTS sucursal_id uuid
         REFERENCES oymcomercial.sucursales(id) ON DELETE RESTRICT', t);
    EXECUTE format(
      'CREATE INDEX IF NOT EXISTS idx_%s_sucursal ON oymcomercial.%I (sucursal_id)', t, t);
  END LOOP;
END $$;
...
DECLARE
  ...
  n     bigint;
BEGIN
  SELECT id INTO v_emp FROM oymcomercial.empresas LIMIT 1;
  SELECT id INTO v_cm  FROM oymcomercial.sucursales
  ...
  FOREACH t IN ARRAY ARRAY[
    ...
    EXECUTE format(
      'UPDATE oymcomercial.%I SET sucursal_id = $1 WHERE sucursal_id IS NULL', t)
      USING v_cm;
    EXECUTE format(
      'SELECT count(*) FROM oymcomercial.%I WHERE sucursal_id IS NULL', t) INTO n;
    IF n > 0 THEN
  ...
  INSERT INTO oymcomercial.usuario_sucursales (usuario_id, sucursal_id, empresa_id)
  SELECT u.id, v_cm, v_emp FROM oymcomercial.usuarios u
  ON CONFLICT (usuario_id, sucursal_id) DO NOTHING;

  UPDATE oymcomercial.usuarios
...
DROP INDEX IF EXISTS oymcomercial.idx_productos_empresa_sku;
CREATE UNIQUE INDEX idx_productos_empresa_sucursal_sku
  ON oymcomercial.productos (empresa_id, sucursal_id, sku);

-- productos: código de barras único por sucursal
DROP INDEX IF EXISTS oymcomercial.uq_productos_codigo_barras;
CREATE UNIQUE INDEX uq_productos_codigo_barras
  ON oymcomercial.productos (empresa_id, sucursal_id, codigo_barras)
  WHERE codigo_barras IS NOT NULL;

-- facturas: numero_factura único por sucursal (permite FAC- en ambas)
DROP INDEX IF EXISTS oymcomercial.uq_facturas_empresa_numero;
CREATE UNIQUE INDEX uq_facturas_empresa_sucursal_numero
  ON oymcomercial.facturas (empresa_id, sucursal_id, numero_factura);

-- nota_credito: correlativo único por sucursal
DROP INDEX IF EXISTS oymcomercial.nota_credito_numero_empresa_uq;
CREATE UNIQUE INDEX nota_credito_numero_empresa_sucursal_uq
  ON oymcomercial.nota_credito (empresa_id, sucursal_id, numero)
...
DROP INDEX IF EXISTS oymcomercial.uq_categorias_productos_empresa_nombre;
CREATE UNIQUE INDEX uq_categorias_productos_empresa_sucursal_nombre
  ON oymcomercial.categorias_productos (empresa_id, sucursal_id, lower(btrim(nombre)));
...
BEGIN
  SELECT count(*) INTO v_suc FROM oymcomercial.sucursales;
  IF v_suc <> 2 THEN RAISE EXCEPTION 'ABORT: se esperaban 2 sucursales, hay %', v_suc; END IF;

  SELECT count(*) INTO v_us FROM oymcomercial.usuarios;
  SELECT count(*) INTO v_sin_acceso FROM oymcomercial.usuarios u
   WHERE NOT EXISTS (SELECT 1 FROM oymcomercial.usuario_sucursales us WHERE us.usuario_id = u.id);
  IF v_sin_acceso > 0 THEN
    RAISE EXCEPTION 'ABORT: % de % usuarios sin acceso a ninguna sucursal', v_sin_acceso, v_us;
  END IF;

  SELECT count(*) INTO v_sin_default FROM oymcomercial.usuarios
   WHERE sucursal_predeterminada_id IS NULL;
