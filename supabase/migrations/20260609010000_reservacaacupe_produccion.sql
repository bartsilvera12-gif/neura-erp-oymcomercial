-- Producción / fabricación desde recetas (solo schema oymcomercial). Idempotente.
...
-- 3) producciones ----------------------------------------------------------
CREATE TABLE IF NOT EXISTS oymcomercial.producciones (
...
CREATE TABLE IF NOT EXISTS oymcomercial.produccion_items (
  id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  empresa_id          uuid NOT NULL,
  produccion_id       uuid NOT NULL REFERENCES oymcomercial.producciones(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_producciones_empresa_fecha ON oymcomercial.producciones (empresa_id, fecha DESC);
CREATE INDEX IF NOT EXISTS idx_produccion_items_produccion ON oymcomercial.produccion_items (produccion_id);
CREATE INDEX IF NOT EXISTS idx_mov_produccion_id ON oymcomercial.movimientos_inventario (produccion_id);
