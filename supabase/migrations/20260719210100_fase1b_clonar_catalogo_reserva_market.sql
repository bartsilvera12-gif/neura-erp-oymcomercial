...
--   DELETE FROM oymcomercial.productos
--    WHERE sucursal_id = (SELECT id FROM oymcomercial.sucursales WHERE codigo='RESERVA_MARKET');
...
BEGIN
  ...
  SELECT id INTO v_cm FROM oymcomercial.sucursales WHERE empresa_id=v_emp AND codigo='CASA_MATRIZ';
  SELECT id INTO v_mk FROM oymcomercial.sucursales WHERE empresa_id=v_emp AND codigo='RESERVA_MARKET';
  ...
  SELECT count(*) INTO n_ya FROM oymcomercial.productos WHERE sucursal_id = v_mk;
  IF n_ya > 0 THEN
  ...
  WITH nuevas AS (
    INSERT INTO oymcomercial.categorias_productos
    ...
    SELECT gen_random_uuid(), c.empresa_id, v_mk, c.nombre, c.codigo, c.descripcion, NULL, c.activo
      FROM oymcomercial.categorias_productos c
     WHERE c.sucursal_id = v_cm
    RETURNING id, lower(btrim(nombre)) AS k
  )
  INSERT INTO map_cat (origen, clon)
  SELECT o.id, n.id
    FROM oymcomercial.categorias_productos o
    JOIN nuevas n ON n.k = lower(btrim(o.nombre))
   WHERE o.sucursal_id = v_cm;

  UPDATE oymcomercial.categorias_productos c
     SET parent_id = m2.clon
    FROM map_cat m1
    JOIN oymcomercial.categorias_productos o ON o.id = m1.origen
  ...
  SELECT gen_random_uuid(), pc.empresa_id, v_mk, mp.clon, mc.clon, pc.es_principal
    FROM oymcomercial.producto_categorias pc
...
BEGIN
  SELECT id INTO v_cm FROM oymcomercial.sucursales WHERE codigo='CASA_MATRIZ';
  SELECT count(*), coalesce(sum(stock_actual),0) INTO n_cm, n_stock
    FROM oymcomercial.productos WHERE sucursal_id = v_cm;
  IF n_cm <> 499 THEN
