-- Recibos de dinero (comprobante interno NO fiscal). Solo schema oymcomercial. Idempotente.
-- No toca stock, deuda, ventas, cobros, SIFEN ni otros schemas.

CREATE TABLE IF NOT EXISTS oymcomercial.recibos_dinero (
...
ALTER TABLE oymcomercial.recibos_dinero
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_recibos_empresa_numero
  ON oymcomercial.recibos_dinero (empresa_id, numero_recibo);

-- Anti-duplicado: un recibo por cobro de CxC.
CREATE UNIQUE INDEX IF NOT EXISTS uq_recibos_cobro
  ON oymcomercial.recibos_dinero (cobro_cliente_id)
