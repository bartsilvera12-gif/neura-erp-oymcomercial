...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS oymcomercial.transferencias_inventario (
  id                    uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  empresa_id            uuid NOT NULL REFERENCES oymcomercial.empresas(id) ON DELETE CASCADE,
  numero                text NOT NULL,
  sucursal_origen_id    uuid NOT NULL REFERENCES oymcomercial.sucursales(id) ON DELETE RESTRICT,
  sucursal_destino_id   uuid NOT NULL REFERENCES oymcomercial.sucursales(id) ON DELETE RESTRICT,
  estado                text NOT NULL DEFAULT 'pendiente',
  observacion_solicitud text,
  motivo_rechazo        text,
  solicitada_por        uuid REFERENCES oymcomercial.usuarios(id) ON DELETE SET NULL,
  aprobada_por          uuid REFERENCES oymcomercial.usuarios(id) ON DELETE SET NULL,
  despachada_por        uuid REFERENCES oymcomercial.usuarios(id) ON DELETE SET NULL,
  recibida_por          uuid REFERENCES oymcomercial.usuarios(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_transf_empresa        ON oymcomercial.transferencias_inventario (empresa_id);
CREATE INDEX IF NOT EXISTS idx_transf_origen         ON oymcomercial.transferencias_inventario (sucursal_origen_id);
CREATE INDEX IF NOT EXISTS idx_transf_destino        ON oymcomercial.transferencias_inventario (sucursal_destino_id);
CREATE INDEX IF NOT EXISTS idx_transf_estado         ON oymcomercial.transferencias_inventario (empresa_id, estado);

DROP TRIGGER IF EXISTS transferencias_inventario_updated_at ON oymcomercial.transferencias_inventario;
CREATE TRIGGER transferencias_inventario_updated_at
  BEFORE UPDATE ON oymcomercial.transferencias_inventario
  FOR EACH ROW EXECUTE FUNCTION oymcomercial.set_updated_at();

-- ---------------------------------------------------------------------------
-- 3) transferencias_inventario_items (líneas)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS oymcomercial.transferencias_inventario_items (
  id                          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  transferencia_id            uuid NOT NULL REFERENCES oymcomercial.transferencias_inventario(id) ON DELETE CASCADE,
  empresa_id                  uuid NOT NULL REFERENCES oymcomercial.empresas(id) ON DELETE CASCADE,
  -- La solicitud nace con el producto de la sucursal DESTINO (la que pide).
  producto_destino_id         uuid NOT NULL REFERENCES oymcomercial.productos(id) ON DELETE RESTRICT,
  -- Equivalente en la sucursal ORIGEN (mismo SKU). Nullable hasta resolverse.
  producto_origen_id          uuid REFERENCES oymcomercial.productos(id) ON DELETE RESTRICT,
  sku_snapshot                text,
...
CREATE INDEX IF NOT EXISTS idx_transf_item_transf   ON oymcomercial.transferencias_inventario_items (transferencia_id);
CREATE INDEX IF NOT EXISTS idx_transf_item_prod_dst ON oymcomercial.transferencias_inventario_items (producto_destino_id);
CREATE INDEX IF NOT EXISTS idx_transf_item_prod_org ON oymcomercial.transferencias_inventario_items (producto_origen_id);

DROP TRIGGER IF EXISTS transferencias_items_updated_at ON oymcomercial.transferencias_inventario_items;
CREATE TRIGGER transferencias_items_updated_at
  BEFORE UPDATE ON oymcomercial.transferencias_inventario_items
  FOR EACH ROW EXECUTE FUNCTION oymcomercial.set_updated_at();
...
DROP POLICY IF EXISTS transf_empresa_isolation ON oymcomercial.transferencias_inventario;
CREATE POLICY transf_empresa_isolation ON oymcomercial.transferencias_inventario
  USING (empresa_id = oymcomercial.empresa_id_actual())
  WITH CHECK (empresa_id = oymcomercial.empresa_id_actual());

DROP POLICY IF EXISTS transf_items_empresa_isolation ON oymcomercial.transferencias_inventario_items;
CREATE POLICY transf_items_empresa_isolation ON oymcomercial.transferencias_inventario_items
  USING (empresa_id = oymcomercial.empresa_id_actual())
  WITH CHECK (empresa_id = oymcomercial.empresa_id_actual());
...
WHERE NOT EXISTS (SELECT 1 FROM oymcomercial.modulos WHERE slug = 'reposicion');
...
BEGIN
  SELECT pg_get_constraintdef(con.oid) INTO v_chk
   ...
   WHERE n.nspname='oymcomercial' AND t.relname='movimientos_inventario'
  ...
  SELECT count(*) INTO v_mod FROM oymcomercial.modulos WHERE slug='reposicion';
